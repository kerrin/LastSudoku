...
{
    /**
     * Unsolve handler for the Swordfish rule.
     *
     * Swordfish is candidate-elimination only; this handler removes a value only
     * when the resulting board creates a Swordfish deduction that trims the
     * removed cell.
     */
    public class SwordFishUnsolveHandler : IUnsolveHandler
    {
        private readonly SwordFishRule _rule = new SwordFishRule();

        public string RuleName => nameof(SwordFishRule);
