...
namespace Sudoku.Solver.Rules
{
     ...
     * two colours enabled.
     */
    public class ForcingChainUnitRule : CachedRuleBase
    {
        ...
        public override string Name => "Forcing Chain (Unit)";
        ...
        private ForcingPlan FindPlan(Board board)
        {
            ...
            // We need to check each candidate in order:
            foreach (var seed in orderedCandidates)
            {
                // For each candidate, check all seen candidates of the same digit in the row, then column, then box...
                var cell = board.Cells[GetRow(board, seed), GetColumn(board, seed)];
                var rowCandidates = new List<Cell>();
                foreach (Cell c in board.GetRow(cell.Row)) if (!ReferenceEquals(c, cell)) rowCandidates.Add(c);
                var columnCandidates = new List<Cell>();
                foreach (Cell c in board.GetColumn(cell.Column)) if (!ReferenceEquals(c, cell)) columnCandidates.Add(c);
                var boxCandidates = new List<Cell>();
                foreach (Cell c in board.GetBox(cell.Box)) if (!ReferenceEquals(c, cell)) boxCandidates.Add(c);
                var seenUnits = new List<List<Cell>>() { rowCandidates, columnCandidates, boxCandidates };
                foreach(var unit in seenUnits)
                {
                    var plansForUnit = new List<ForcingPlan>();
                    foreach(var peerCell in unit)
                    {
                        if (peerCell == null || peerCell.Value.HasValue || peerCell.Candidates == null) continue;
                        if (!peerCell.Candidates.Contains(GetDigit(board, seed))) continue;

                        int peerCandidate = MakeCandidateKey(board, peerCell.Row, peerCell.Column, GetDigit(board, seed));
                        if (peerCandidate == seed) continue;

                        // Find a common-conclusion forcing chain.
                        // e.g. for all candidates of the same digit in the same row, assume each one is true and propagate, then check for common conclusions.
                        ...
                        plansForUnit.Add(plan);
                    }
                    var combinedPlan = CombinePlansForUnit(plansForUnit);
         ...
         */
        private ForcingPlan CombinePlansForUnit(List<ForcingPlan> plans)
        {
