...
namespace Sudoku.Tests.Editor
{
    ...
    {
        [Test]
        public void HiddenTripleRule_RowHiddenTriple_WithMixedDigitFrequencies_RemovesOtherCandidatesFromTripleCells()
        {
            ...
            // Digits 1,2,3 are collectively limited to three cells (0,0), (0,4), (0,8),
            // but each digit does not appear in all three cells.
            board.Cells[0, 0].Candidates.UnionWith(new[] { 1, 2, 7 });
            board.Cells[0, 4].Candidates.UnionWith(new[] { 1, 3, 8 });
            board.Cells[0, 8].Candidates.UnionWith(new[] { 2, 3, 9 });
