...
{
    public class HiddenPairUnsolveHandlerTests
    {
        ...
        [Test]
        public void GetHandler_ForHiddenPairRule_ReturnsHiddenPairUnsolveHandler()
        {
            var handler = UnsolveHandlerRegistry.GetHandler(new HiddenPairRule());

            Assert.IsInstanceOf<HiddenPairUnsolveHandler>(handler);
        }
        ...
        public void TryUnsolve_OnFullySolvedBoard_ReturnsNoApplicableMove_AndDoesNotMutateBoard()
        {
            ...
            var handler = new HiddenPairUnsolveHandler();
        ...
        [Test]
        public void TryUnsolve_WhenHiddenPairOpportunityExists_RemovesExactlyOneValue()
        {
            for (int seed = 0; seed < 80; seed++)
            {
                ...
                Assert.IsTrue(board.IsValid(), "Board must stay valid after HiddenPair unsolve.");
                return;
            }

            Assert.Inconclusive(
                "No Hidden Pair unsolve opportunity was found in tested seeds. " +
