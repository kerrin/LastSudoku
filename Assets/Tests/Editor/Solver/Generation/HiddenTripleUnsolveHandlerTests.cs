...
{
    public class HiddenTripleUnsolveHandlerTests
    {
        ...
        [Test]
        public void GetHandler_ForHiddenTripleRule_ReturnsHiddenTripleUnsolveHandler()
        {
            var handler = UnsolveHandlerRegistry.GetHandler(new HiddenTripleRule());

            Assert.IsInstanceOf<HiddenTripleUnsolveHandler>(handler);
        }
        ...
        public void TryUnsolve_OnFullySolvedBoard_ReturnsNoApplicableMove_AndDoesNotMutateBoard()
        {
            ...
            var handler = new HiddenTripleUnsolveHandler();
        ...
        [Test]
        public void BuildCandidateList_WhenHiddenTripleOpportunityExists_ReturnsAtLeastOneCell()
        {
            for (int seed = 0; seed < 200; seed++)
            {
                ...
                Assert.IsTrue(board.IsValid(), "Board must stay valid after HiddenTriple unsolve.");
                return;
            }

            Assert.Inconclusive(
                "No Hidden Triple unsolve opportunity was found in tested seeds. " +
