...
{
    public class PointingPairUnsolveHandlerTests
    {
        ...
        [Test]
        public void GetHandler_ForPointingPairRule_ReturnsPointingPairUnsolveHandler()
        {
            var handler = UnsolveHandlerRegistry.GetHandler(new PointingPairRule());

            Assert.IsInstanceOf<PointingPairUnsolveHandler>(handler);
        }
        ...
        public void TryUnsolve_OnFullySolvedBoard_ReturnsNoApplicableMove_AndDoesNotMutateBoard()
        {
            ...
            var handler = new PointingPairUnsolveHandler();
        ...
        [Test]
        public void BuildCandidateList_WhenPointingPairOpportunityExists_ReturnsAtLeastOneCell()
        {
            for (int seed = 0; seed < 160; seed++)
            {
                ...
                Assert.IsTrue(board.IsValid(), "Board must stay valid after PointingPair unsolve.");
                return;
            }

            Assert.Inconclusive(
                "No Pointing Pair unsolve opportunity was found in tested seeds. " +
